...
class ChartLinePainter extends BasePainter {
  ...
  void _drawXy(Canvas canvas, Size size) {
    ...
    if (isShowXy) {
      // x
      canvas.drawLine(
          Offset(startX, startY), Offset(endX + basePadding, startY), paint);
      // y
      canvas.drawLine(
          Offset(startX, startY), Offset(startX, endY - basePadding), paint);
    }
    if (isShowBorderTop) {
      //顶部水平边界线
      canvas.drawLine(Offset(startX, endY - basePadding),
          Offset(startX + basePadding, endY - basePadding), paint);
    }
  ...
  void drawRuler(Canvas canvas, Paint paint) {
    if (chartBeans != null && chartBeans.length > 0) {
      int length = chartBeans.length > 7 ? 7 : chartBeans.length;
      double dw = _fixedWidth / (length - 1);
      double dh = _fixedHeight / (length - 1);
      for (var i = 0; i < length; i++) {
        ...
        // x 轴刻度
        canvas.drawLine(Offset(startX + dw * i, startY),
            Offset(startX + dw * i, startY - basePadding), paint);
      }
  ...
  void _drawLine(Canvas canvas, Size size) {
    ...
    // 绘制阴影  注意LinearGradient这里需要指定方向，默认为从左到右
      ...
      if (isShowPressedHintLine) {
        canvas
          ...
          ..drawLine(
              ...
              Offset(pointer.dx, endX - basePadding),
              hintLinePaint..strokeWidth = pressedHintLineWidth);
      }
